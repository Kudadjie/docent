...
def test_check_cli_tool_ok() -> None:
    ...
    with (
        ...
        label, status, version, detail = _check_cli_tool(
            "Python", ["python", "--version"], "install hint"
        )
    assert status == "OK"
...
def test_check_cli_tool_not_found() -> None:
    with patch("shutil.which", return_value=None):
        label, status, version, detail = _check_cli_tool(
            "mytool", ["mytool", "--version"], "install mytool"
        )
    assert status == "FAIL"
...
def test_check_alphaxiv_shows_update_hint() -> None:
    ...
    fake_update = UpdateInfo(
        tool="alphaxiv-py", current="0.5.0", latest="99.0.0", upgrade_cmd="uv add alphaxiv-py"
    )
...
def test_check_notebooklm_py_shows_update_hint() -> None:
    ...
    fake_update = UpdateInfo(
        tool="notebooklm-py", current="0.4.1", latest="99.0.0", upgrade_cmd="uv add notebooklm-py"
    )
    with (
        patch("shutil.which", return_value="/usr/bin/notebooklm"),
        patch("subprocess.run", return_value=_nlm_auth_ok_subprocess()),
