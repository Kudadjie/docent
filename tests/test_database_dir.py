"""Unit tests for PaperPipeline._require_database_dir.
...
from docent.tools.paper import PaperPipeline
...
def test_returns_configured_path(tmp_docent_home, tmp_path):
    ...
    db.mkdir()
    ctx = _make_context()
    ctx.settings.paper.database_dir = db

    folder, err = PaperPipeline()._require_database_dir(ctx)
...
def test_invalid_typed_path_returns_error_and_does_not_persist(
    ...
    monkeypatch.setattr(
        "docent.tools.paper.prompt_for_path",
        lambda *_a, **_kw: bogus,
    )

    ctx = _make_context()
    assert ctx.settings.paper.database_dir is None
