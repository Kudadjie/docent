...
`ready-to-read` stamps `started` on first transition; `done` stamps `finished`.
...
from docent.tools.paper import IdOnlyInputs, PaperPipeline
...
def test_ready_to_read_sets_started(tmp_docent_home, seed_queue_entry):
    tool = PaperPipeline()
    seeded = seed_queue_entry(tool, title="X", authors="Smith, J", year=2024, doi="10.1/x")
    eid = seeded["id"]

    result = tool.ready_to_read(IdOnlyInputs(id=eid), _ctx())
    assert result.ok
...
def test_done_sets_finished(tmp_docent_home, seed_queue_entry):
    ...
    tool.ready_to_read(IdOnlyInputs(id=eid), _ctx())
...
def test_started_not_overwritten_on_re_read(tmp_docent_home, seed_queue_entry):
    ...
    first = tool.ready_to_read(IdOnlyInputs(id=eid), _ctx())
    first_started = first.entry.started

    second = tool.ready_to_read(IdOnlyInputs(id=eid), _ctx())
