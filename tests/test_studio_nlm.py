...
class TestNlmRun:
    ...
    def test_captures_stdout_and_stderr(self):
        with (
            patch(
                "docent.bundled_plugins.studio._notebook._nlm_exe", return_value="/bin/notebooklm"
            ),
...
class TestNlmAuthOk:
    ...
    def test_returns_false_on_nonzero_returncode(self):
        with (
            ...
            patch(
                "docent.bundled_plugins.studio._notebook._nlm_run",
    ...
    def test_returns_true_when_list_returns_empty_array(self):
        with (
            ...
            patch("docent.bundled_plugins.studio._notebook._nlm_run", return_value=(0, "[]", "")),
        ):
            assert _nlm_auth_ok() is True
    ...
    def test_returns_false_on_invalid_json(self):
        ...
        with (
            ...
            patch(
                "docent.bundled_plugins.studio._notebook._nlm_run", return_value=(0, "not json", "")
            ),
        ):
            assert _nlm_auth_ok() is False
...
class TestNlmCreateNotebook:
    ...
    def test_returns_none_on_nonzero_returncode(self):
        with patch(
            "docent.bundled_plugins.studio._notebook._nlm_run", return_value=(1, "", "failed")
        ):
            result = _nlm_create_notebook("My Notebook")
    ...
    def test_passes_title_and_json_flag(self):
        with patch(
            "docent.bundled_plugins.studio._notebook._nlm_run",
            return_value=(0, json.dumps({"id": "nb1"}), ""),
        ) as mock_run:
...
class TestNlmAddSource:
    def test_returns_zero_on_success(self):
        with patch("docent.bundled_plugins.studio._notebook._nlm_run", return_value=(0, "", "")):
    ...
    def test_passes_notebook_id_flag(self):
        with patch(
            "docent.bundled_plugins.studio._notebook._nlm_run", return_value=(0, "", "")
        ) as mock_run:
