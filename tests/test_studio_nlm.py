...
class TestNlmRun:
    def test_returns_minus1_when_exe_not_found(self):
        with patch("docent.bundled_plugins.studio._nlm_exe", return_value=None):
...
class TestNlmAuthOk:
    ...
    def test_returns_false_on_nonzero_returncode(self):
        with patch("docent.bundled_plugins.studio._nlm_run", return_value=(1, "", "auth error")):
            assert _nlm_auth_ok() is False

    def test_returns_true_when_list_succeeds_with_array(self):
        notebooks = [{"id": "nb1", "title": "My Notebook"}]
        with patch("docent.bundled_plugins.studio._nlm_run",
