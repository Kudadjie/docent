...
- [Harness engineering principles](harness_principles.md) — eval harness shipped 2026-05-15 (tests/golden/studio/); deferred ⏳ rows: verification loops, prompt versioning, lifecycle hooks, model-facing surface review
...
- [API route rule](feedback_two_file_api_sync.md) — new routes go in ui_routes/ only; Next.js dev proxies /api/* to FastAPI via next.config.ts rewrites — no duplicate Next.js route needed
...
- [Studio tool routing](feedback_research_tool_routing.md) — Feynman is default backend; `--backend docent` is the explicit alternate; `--backend free` is no-cost aggregation; no litellm fallback
...
- [Release plan](release_plan.md) — single bundled track (CLI + UI ship together); v2.0.0 shipped 2026-05-23; v2.0.4 latest; v2.1.0 next (schema-driven forms, Zotero bridge, plugin docs)
- [Studio real-life tests](tasks/done/studio_real_life_tests.md) — 45-item manual test checklist; covers CLI, free tier, guide-files, MCP synthesis prompt, and paid backends; all passed v2.0.0
- [UI test checklist](tasks/v120_ui_tests.md) — 60+ manual UI checks across all pages; one item still open: bell dropdown mark-all-as-read
