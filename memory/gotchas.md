...
- **2026-05-30 — Tests that read the global tool `_REGISTRY` must isolate it.** `build_mcp_tools()` (and anything via `all_tools()`) reads `docent.core.registry._REGISTRY`, a module-global. `test_tool_abc.py` / `test_dispatcher.py` register throwaway fixture tools (`fixture__run`, `test-ping-xyz__pong`, `test-stream-xyz__stream`) into it, which then leak into any later test that introspects the registry. Symptom: a test passes in isolation but fails in the full suite (order-dependent). Fix pattern (see `tests/test_mcp_surface.py::_mcp_tools`): snapshot `dict(registry._REGISTRY)`, `.clear()`, fresh `discover_tools()`+`load_plugins()`, do the work, then restore in a `finally`.
