...
- **Mode 1 — NL → existing action** ("summarize my queue" → `docent paper stats`). Build first; achievable today with a small classifier + the existing `all_tools()` registry. Ship as part of Phase 2 launch.
...
```
[done]    Steps 1-10.5
[next]    Step 11  (paper sync + minimal MCP)
          Step 12  (~/.docent/plugins/ discovery)        }  user-extensibility
...
[then]    FastAPI layer + JSON wire format               }  Phase 2
          React frontend (schema-driven, shapes-rendered)}
